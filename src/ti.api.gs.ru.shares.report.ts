...
import { RealAccount, TinkoffInvestApi, CandlesLoader } from 'tinkoff-invest-api';
import { CandleInterval, GetLastPricesResponse, HistoricCandle, LastPrice } from 'tinkoff-invest-api/cjs/generated/marketdata';
...
import { ACCOUNT } from './ti.api.service.utils';
...
const get_min_max_by_hist_candles = async (candles: HistoricCandle[]): Promise<MinPriceShare> => {
    let curr_max_high = api.helpers.toNumber(candles[0]?.high)!;
    let curr_min_low = api.helpers.toNumber(candles[0]?.low)!;
    let curr_time: any;
    ...
    candles.forEach( (candle:HistoricCandle) => {
        let curr_low = api.helpers.toNumber(candle.low)!;
        let curr_high = api.helpers.toNumber(candle.high)!;
...
const get_min_close_price_by_hist_candles = async (candles: HistoricCandle[]): Promise<MinClosePriceShare> => {
    let curr_min_low_close_price = api.helpers.toNumber(candles[0]?.close)!;
    let curr_time: any;
    ...
    candles.forEach( (candle:HistoricCandle) => {
        let curr_low = api.helpers.toNumber(candle.close)!;
    ...
    figies.forEach( async (figi:string) => {
        ...
        try {
            let {candles} = await api.marketdata.getCandles({
              ...
              const currPrice = api.helpers.toNumber(obj?.price);
...
const exec = async () => {
    ...
    const sector_tickers_figies : ShareStruct[] = require('../all_fix.json');
