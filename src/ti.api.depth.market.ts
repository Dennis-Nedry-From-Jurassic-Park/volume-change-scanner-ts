...
import { get_share_by_ticker, MARKET_DEPTH, waitMarketStreamEvent } from './ti.api.service.utils';
...
const logger = winston.createLogger({
    ...
    transports: [
      new winston.transports.File({ filename: 'error_market_depth.log', level: 'error' }),
      new winston.transports.File({ filename: 'combined_market_depth.log', level: 'debug' }),
    ],
  });

const logger_candles = winston.createLogger({
    level: 'debug',
    format: winston.format.json(),
    defaultMeta: { service: 'market-depth-service' },
    transports: [
...
const get_candles_historical = async (ticker:string) => {
  ...
  let newCandles = candles.map(object => {
      return {
        ...object,
        open: api.helpers.toNumber(object.open),
        high: api.helpers.toNumber(object.high),
        low: api.helpers.toNumber(object.low),
        close: api.helpers.toNumber(object.close),
      };
  });
