...
const exec = async () => {
    ...
    for(const codec of codecs2) {
        const create_table = `
            create table if not exists ${table_name} (x ${type}) engine=MergeTree
        `; // сжатие LZ4

        let resp = await clickhouse.query(create_table).toPromise();
        assert(resp['r'] === 1, `not ok. table ${table_name} not created.`)
        console.log(codec)
        const alter_query = `alter table ${table_name} modify column x ${type} ${codec};`
        ...
        const insert_q = `insert into ${table_name} (n) select number from numbers(${rows}) settings max_block_size=1000000;`
        resp = await clickhouse.query(insert_q).toPromise();
        let json_resp = JSON.parse(resp.toString());
        const insert_q_elapsed = json_resp.statistics.elapsed
